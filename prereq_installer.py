...
MODULES_TO_INSTALL = ['numpy', 'time', 'random', 'curses']
CONFLICTING_FILE_NAMES = {'numpy.py', 'time.py', 'random.py', 'curses.py'}
PATH = sys.executable
...
def install_module_for_user(module):
    ...
    subprocess.run([PATH] + f'-m pip install --user {module} --force-reinstall'.split(), check=True)
